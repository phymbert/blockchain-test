 ...
 * 
 * <p>Note that this is not the same base58 as used by Flickr,
 *  which you may find referenced around Internet.
 * 
 ...
 * </ul>
 * 
 * <p>However, note that the encoding/decoding runs in O(n&sup2;) time,
 * so it is not useful for large data.
 * 
...
// input.
  ...
  public static byte[] decode(String input) throws ParseException {
    ...
    for (int i = 0; i < input.length(); ++i) {
      ...
      int digit = ca < 128 ? INDEXES[ca] : -1;
      if (digit < 0) {
        throw new ParseException("Illegal character " + ca + " at position " + i, i);
      }
